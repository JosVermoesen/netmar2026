...
@Component({
    ...
    imports: [SupplierItem, MatButton, MatIcon, MatMenu, MatSelectionList, MatListOption, MatMenuTrigger, MatPaginator, FormsModule, EmptyState],
...
export class Suppliers implements OnInit {
    ...
    suppliers = signal<Pagination<Supplier> | null>(null);
    sortOptions = [
    ...
    onSearchChange() {
        this.supplierParams.pageNumber = 1;
        this.supplierService.getSuppliers(this.supplierParams).subscribe({
            next: (response) => this.suppliers.set(response),
            error: (error) => console.log('Error fetching suppliers:', error)
        });
