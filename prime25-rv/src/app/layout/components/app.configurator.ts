...
import { booleanAttribute, Component, computed, inject, Input, model, OnInit, PLATFORM_ID, Signal, ChangeDetectionStrategy } from '@angular/core';
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, SelectButtonModule, DrawerModule, ToggleSwitchModule, RadioButtonModule, Tooltip, TranslateModule],
    ...
    template: `
        ...
        <p-drawer [visible]="visible()" (onHide)="onDrawerHide()" position="right" [transitionOptions]="'.3s cubic-bezier(0, 0, 0.2, 1)'" styleClass="layout-config-sidebar w-80" header="{{ 'CONFIG.Settings' | translate }}">
            <span class="text-lg font-semibold">{{ 'CONFIG.Language' | translate }}</span>
            <div class="flex flex-wrap gap-2 mb-4">
                @for (lang of componentLanguages; track lang) {
                    <div>
                        <div class="flex items-center gap-2 hover:cursor-pointer">
                            <span pTooltip="{{ lang.code }}" tooltipPosition="bottom">
                                <img (click)="changeLanguage(lang.code)" src="https://primefaces.org/cdn/primeng/images/flag/flag_placeholder.png" [class]="'flag flag-' + lang.flag" width="30" placeholder="Bottom" />
                            </span>
                        </div>
                    </div>
                }
                    ...
                    <div class="pt-2 flex gap-2 flex-wrap">
                        @for (primaryColor of primaryColors(); track primaryColor.name) {
                            <button
                                ...
                                class="cursor-pointer w-6 h-6 rounded-full flex shrink-0 items-center justify-center p-0 outline-hidden outline-offset-1"
                                [style]="{
...
export class AppConfigurator implements OnInit {
    ...
    // VSOFT portion - language switcher
